...
// BuyItem now expects a pointer to a struct with Inventaire, Gold fields
func BuyItem(c interface{}, it Item) {
	// Type assertion for expected struct
	type inventoryGold interface {
		GetInventaire() *[]Item
		GetGold() *int
	}
	cg, ok := c.(inventoryGold)
	if !ok {
		fmt.Println("Type non supporté pour l'achat d'item.")
		return
	}
	inventaire := cg.GetInventaire()
	gold := cg.GetGold()
	...
	if *gold >= it.Price {
		*gold -= it.Price
		*inventaire = append(*inventaire, it)
...
func ShopMenu(c interface{}) {
	...
	for {
		fmt.Println("\n================ BOUTIQUE =================")
		// fmt.Printf("Or actuel : %d\n", c.Gold) // Gold should be accessed via GetGold()
		cg, ok := c.(interface{ GetGold() *int })
		if ok {
			fmt.Printf("Or actuel : %d\n", *cg.GetGold())
		} else {
			fmt.Println("Or actuel : ?")
		}
		fmt.Println("-------------------------------------------")
...
func AddItemToInventory(inventaire *[]Item, newItem Item) {
	if CanAddItem(*inventaire) {
		*inventaire = append(*inventaire, newItem)
	} else {
